...
namespace GenericChess
{
    class Queen : IPiece
    {
        ...
        public bool IsMoveValid(Board board, Vector2 endPosition)
        {
            ...
            var finalPosValid = false;
            if ((delta.x == delta.y || delta.x == -delta.y)) finalPosValid = true; //diagonal
            if ((delta.x == 0 && delta.y != 0) || (delta.y == 0 && delta.x != 0)) finalPosValid = true; // Straight                                                                       
            if (!finalPosValid) return false; //Return when final position is not straight or diagonal
